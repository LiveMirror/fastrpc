...
{
	if (lis_fd <= 0) {
		EPPOOLLOG(2, "invalid listen fd %d.", lis_fd);
		return -1;
	}
...
int ependingpool::set_event_callback(int event, event_callback_t* callback)
{
	if (NULL == m_callbacklist) {
		EPPOOLLOG(1, "m_callbacklist is NULL");
		return -1;
	}
	if (event < 0 || event >= SOCK_EVENT_NUM) {
		EPPOOLLOG(2, "event must between [0, %d]", SOCK_EVENT_NUM);
		return -1;
	}
	if (m_callbacklist[event] != NULL) {
		EPPOOLLOG(2, "event %d is no NULL", event);
	}
...
{
	if (num <= 0) {
		EPPOOLLOG(2, "invalid socket number %d. sock_num keep unchanged, %d.",
				num, m_sock_num);
		return -1;
	}
	...
	m_ay_sock = (sock_item_t *)calloc((size_t)m_sock_num, sizeof(sock_item_t));
	if (!m_ay_sock) {
		EPPOOLLOG(2, "m_ay_sock calloc memory failed. [%d]: %m.", errno);
		return -1;
	}
	...
	if (NULL == m_ay_events) {
		EPPOOLLOG(2, "m_ay_events calloc memory failed. [%d]: %m.", errno);
		return -1;
	}

	m_epfd = epoll_create(m_sock_num);
	if (m_epfd < 0) {
		EPPOOLLOG(2, "create epoll descriptor failed. [%d]: %m.", errno);
		return -1;
	}
...
{
	if (timeo <= 0) {
		EPPOOLLOG(2, "invalid timeout %d timeout. timeout keep unchanged, %d.",
				timeo, m_conn_timeo);
		return -1;
	}
...
{
	if (timeo <= 0) {
		EPPOOLLOG(2, "invalid timeout %d read timeout. timeout keep unchanged, %d.",
				timeo, m_read_timeo);
		return -1;
	}
...
{
	if (timeo <= 0) {
		EPPOOLLOG(2, "invalid timeout %d write timeout. timeout keep unchanged, %d.",
				timeo, m_write_timeo);
		return -1;
	}
...
{
	if (sock < 0) {
		EPPOOLLOG(1, "insert socket fail, invalid socket %d.", sock);
		return -1;
	}
	...
	if (-1 == current_offset) {
		...
		EPPOOLLOG(2, "insert socket[%d] fail, socket array[max_size:%d] overflow.",
				sock, m_sock_num);
		if (safe) {
	...
	EPPOOLLOG(10, "socket %d inserted into sock array[offset:%d] [sock_len:%d].", sock,
...
int ependingpool::fetch_item(int *handle, int *sock)
{
	...
	--m_free_thr;
	if (!pool_run) {
		EPPOOLLOG(10, "pool is stop");
		goto failure;
	}
	//获取句柄
	if (queue_pop(&fetch_handle) < 0) {
		EPPOOLLOG(2, "get invalid handle [%d] from queue.", fetch_handle);
		goto failure;
	}
	...
	if (fetch_sock < 0) {
		EPPOOLLOG(1, "get invalid socket %d from queue.", fetch_sock);
		goto failure;
	}

	if (m_ay_sock[fetch_handle].sock_status != BUSY) {
		EPPOOLLOG(2, "get error handle %d socket %d from queue, status is %d",
	...
	*sock = fetch_sock;
	...
	EPPOOLLOG(10, "handle[%d] socket [%d] geted, head=%d, tail=%d, free_thr [%d].",
	...
	if (m_callbacklist[SOCK_FETCH] != NULL) {
		...
		switch (ret) {
			...
			default:
                EPPOOLLOG(2, "SOCK_FETCH callback return unknown ret[%d]", ret);
				reset_item(fetch_handle, false);
				break;
		}
	}
...
int ependingpool::close_ready_socket()
{
	...
	EPPOOLLOG(10, "close ready socket m_sock_len:%d", m_sock_len);
	int ready = 0;
	for (int i = 0; i < m_sock_len; ++i) {
...
int ependingpool::waitforstop(int timeout)
{
	...
	struct timeval tv;
	EPPOOLLOG(10, "queue_len is %d", get_queue_len());
	stop();
	int handle = -1;
	pthread_mutex_lock(&m_mutex);
	while (!queue_empty()) {
	...
	pthread_mutex_unlock(&m_mutex);
	EPPOOLLOG(10, "queue all close, queue is empty");
	...
	EPPOOLLOG(10, "close socket num is %d, queue_len is %d", close_sock, get_queue_len());
	return 0;
}
...
int ependingpool::clear_item(int handle)
{
	...
	if (handle < 0 || handle >= m_sock_len) {
		EPPOOLLOG(2, "invalid handle %d.", handle);
		return -1;
	}

	if (NULL == m_ay_sock) {
		EPPOOLLOG(1, "reset_item invalid m_ay_sock.");
		return -1;
	}

	if (NOT_USED == m_ay_sock[handle].sock_status) {
		EPPOOLLOG(2, "no found socket in handle %d.", handle);
		return 0;
	}
	...
	if (sock < 0) {
		EPPOOLLOG(2, "invalid handle %d sock %d.", handle, sock);
		return -1;
	}
...
int ependingpool::reset_item(int handle, bool keep_alive)
{
	...
	if (!(keep_alive && pool_run)) {	//不保持连接
		// 关闭句柄
		EPPOOLLOG(10, "close sock[%d]", sock);
		...
		if (ret < 0) {
			EPPOOLLOG(2, "close socket handle[%d], %d failed, [%d]: %m.",
					handle, sock, errno);
			return -1;
		}
	...
	EPPOOLLOG(10, "socket %d handle %d, reseted, %s",
...
int ependingpool::write_reset_item(int handle)
{
	...
	EPPOOLLOG(10, "write rest handle[%d]", handle);
	m_ay_sock[handle].last_active = time(NULL);
...
int ependingpool::insert_item_sock(int sock,int &async_offset,void *arg, int flags)
{
	...
	if (arg != NULL) {
		...
		if (m_callbacklist[SOCK_INIT] != NULL) {
			if (m_callbacklist[SOCK_INIT](this, sock, (void**)(&(m_ay_sock[offset].arg))) < 0) {
                EPPOOLLOG(2, "SOCK_INIT callback return < 0");
				clear_item(offset);
				return -1;
			}
		}
	...
	if (pool_epoll_offset_add(offset, events) < 0) {
        ...
        EPPOOLLOG(2, "epoll_del offset[%d] error", offset);
		clear_item(offset);
		return -1;
	}
	return 0;
...
void ependingpool::do_read_event(int offset)
{
	...
	if (offset < 0 || offset >= m_sock_len) {
		EPPOOLLOG(2, "invalid offset %d.", offset);
		return;
	}
	++m_read_socket_num;
	if (m_callbacklist[SOCK_READ] != NULL) {
		EPPOOLLOG(10, "run read callback..., arg [%p]",
	...
	switch (ret) {
		case 0:
            ...
            EPPOOLLOG(2, "queue_in offset[%d] error[%d] sock[%d]", offset, ret, m_ay_sock[offset].sock);
			...
			switch (ret) {
				...
				case 1:
					...
					break;
				default:
                    EPPOOLLOG(2, "SOCK_QUEUEFAIL callback return ret[%d], sock[%d]", ret, m_ay_sock[offset].sock);
					if (m_callbacklist[SOCK_CLOSE] != NULL){
						m_callbacklist[SOCK_CLOSE](this, m_ay_sock[offset].sock,(void**)(&m_ay_sock[offset].arg));
					}
					reset_item(offset, false);
		...
		case 1:
			EPPOOLLOG(10, "read no end ret[%d], sock reset to EPOLLIN",ret);
			pool_epoll_offset_mod(offset, EPOLLIN | EPOLLHUP | EPOLLERR | EPOLLOUT);
			break;

		case 2:
			if (m_callbacklist[SOCK_TODO] != NULL) {
				EPPOOLLOG(10, "read call success, not put into epoll, call"
						"SOCK_TODO");
			...
			switch (ret) {
				...
				default:
					EPPOOLLOG(2, "call back SOCK_TODO run result is err[%d]",ret);
					if (m_callbacklist[SOCK_CLOSE] != NULL){
						m_callbacklist[SOCK_CLOSE](this, m_ay_sock[offset].sock,(void**)(&m_ay_sock[offset].arg));
					}
					reset_item(offset, false);
		...
		case 3:
			EPPOOLLOG(10, "read end call clear item");
			clear_item(offset);
			break;

		default:
			EPPOOLLOG(2, "read call error [%d]", ret);
			if (m_callbacklist[SOCK_CLOSE] != NULL){
				m_callbacklist[SOCK_CLOSE](this, m_ay_sock[offset].sock,(void**)(&m_ay_sock[offset].arg));
			}
			reset_item(offset, false);
...
void ependingpool::do_write_event(int offset)
{
	...
	if (m_callbacklist[SOCK_WRITE] != NULL) {
		EPPOOLLOG(10, "run write callback..., arg [%p]",
				m_ay_sock[offset].arg);
	...
	switch (ret) {
		case 0:
			EPPOOLLOG(10, "write end sock, close sock[%d]",
					m_ay_sock[offset].sock);
			reset_item(offset, true);
			break;
		case 1:
			EPPOOLLOG(10, "write no end ret, continue");//write_reset_item(offset);
			break;
		case 2:
			EPPOOLLOG(10, "write end sock, no close sock[%d]",
					m_ay_sock[offset].sock);
	    	if (m_callbacklist[SOCK_CLOSE] != NULL){
				m_callbacklist[SOCK_CLOSE](this, m_ay_sock[offset].sock,(void**)(&m_ay_sock[offset].arg));
			}
		...
		case 3:
			EPPOOLLOG(10, "write end sock, clear item");
			clear_item(offset);
			break;

		default:
			EPPOOLLOG(2, "write call error [%d]", ret);
			reset_item(offset, false);
			if (m_callbacklist[SOCK_CLOSE] != NULL){
					m_callbacklist[SOCK_CLOSE](this, m_ay_sock[offset].sock,(void**)(&m_ay_sock[offset].arg));
			}
		...
		if (m_ay_events[i].data.fd == -1 && m_listen_fd > 0) {
			...
			while ((work_sock = accept_sock()) > 0) {
				if (insert_item_sock(work_sock,async_offset) < 0) {
					...
					EPPOOLLOG(2, "insert_item_sock work_sock [%d] fail", work_sock);
				}
			}
			...
			//检查发生的事件
			EPPOOLLOG(10, "events handle is [%d]", offset);

			if (m_ay_events[i].events & EPOLLHUP) { //断开
				do_epollhup(offset);  //add by liuyulian;2014-04-09. detech server socket
				EPPOOLLOG(2, "socket %d closed by peer.", m_ay_sock[offset].sock);
				reset_item(offset, false);
			} else if (m_ay_events[i].events & EPOLLERR) {	// 出错
				do_epollerr(offset);
				EPPOOLLOG(2, "socket %d error.", m_ay_sock[offset].sock);
				reset_item(offset, false);
				...
				//EPPOOLLOG(2, "offset %d is close", offset);
				reset_item(offset, false);
			}
		}
	}
...
int ependingpool::accept_sock()
{
	...
	if (m_callbacklist[SOCK_ACCEPT] != NULL) {
		...
		if (work_sock < 0) {
			EPPOOLLOG(2, "accept sock callback fail");
			return -1;
		}
		return work_sock;
	}
	...
	work_sock = accept(m_listen_fd, NULL, NULL);
	if (work_sock < 0) {// accept failed.
		EPPOOLLOG(2, "accept sock fail");
		return -1;
	}
	EPPOOLLOG(10, "accept [%d] success", work_sock);
...
{
	if (!pool_run) {
		EPPOOLLOG(2, "pool is stop");
		return -1;
	}
	int sock = m_ay_sock[offset].sock;
	if (m_ay_sock[offset].sock_status != READY &&
			m_ay_sock[offset].sock_status != READ_BUSY) {
		EPPOOLLOG(2, "status of socket %d offset %d is not ready!", sock, offset);
		return -1;
	}
	...
	if (ret < 0) { //放入失败
		EPPOOLLOG(2, "queue overflow socket %d, queue len %d",
				sock, m_queue_len);
		pthread_mutex_unlock(&m_mutex);
		return -1;
	...
	pthread_mutex_unlock(&m_mutex);
	EPPOOLLOG(10, "socket %d offset %d added into ready queue, head=%d, tail=%d",
...
int ependingpool::check_time_out()
{
		...
		switch (m_ay_sock[i].sock_status) {
			...
			case READY:
				if (current_time >= m_ay_sock[i].last_active + m_conn_timeo) {
					EPPOOLLOG(2, "handle %d time out last_active[%d], m_conn_timeo[%d], current_time[%d]",
			...
			case READ_BUSY:
				if (current_time >= m_ay_sock[i].last_active + m_read_timeo) {
					EPPOOLLOG(2, "handle %d read time out last_active[%d], m_conn_timeo[%d], current_time[%d]",
							i, m_ay_sock[i].last_active, m_conn_timeo, current_time);
			...
			case WRITE_BUSY:
				if (current_time >= m_ay_sock[i].last_active + m_write_timeo) {
					EPPOOLLOG(2, "handle %d write time out last_active[%d], m_conn_timeo[%d], current_time[%d]",
							i, m_ay_sock[i].last_active, m_conn_timeo, current_time);
	...
	if (ready_num == 0 && (busy_num > 0 || read_num > 0 || write_num > 0)) {
		EPPOOLLOG(10, "%d socket READY, %d BUSY, %d READ %d WRITE %d Total",
...
int ependingpool::pool_epoll_wait(int timeout)
{
	...
	if (m_epfd <= 0) {
		EPPOOLLOG(1, "invalid epoll fd %d.", m_epfd);
		return -1;
	}
	...
	while (1) {
		...
		if (nfds < 0) {
			EPPOOLLOG(2, "epoll_wait failed. [%d]: %m", errno);
		}
		break;
	}
	return nfds;
...
int ependingpool::pool_epoll_add(int sock, int fd, int events)
{
	...
	if (epoll_ctl(m_epfd, EPOLL_CTL_ADD, sock, &ev) < 0) {
		EPPOOLLOG(1, "epoll_ctl add socket %d failed. [%d]: %m.", sock, errno);
		return -1;
	}


	EPPOOLLOG(10, "socket %d fd %d add into epoll", sock, fd);
...
int ependingpool::pool_epoll_del(int sock, int fd)
{
	...
	if (epoll_ctl(m_epfd, EPOLL_CTL_DEL, sock, &ev) < 0) {
		EPPOOLLOG(1, "epoll_ctl del socket %d failed. [%d]: %m.", sock, errno);
		return -1;
	}
	EPPOOLLOG(10, "socket %d fd %d delete form epoll", sock, fd);
	return 0;
}
...
int ependingpool::pool_epoll_mod(int sock, int fd, int event)
{
	...
	if (epoll_ctl(m_epfd, EPOLL_CTL_MOD, sock, &ev) < 0) {
		EPPOOLLOG(1, "epoll_mod mod socket %d failed. [%d]: %m.", sock, errno);
		return -1;
	}


	EPPOOLLOG(10, "socket %d fd %d mod in epoll", sock, fd);
