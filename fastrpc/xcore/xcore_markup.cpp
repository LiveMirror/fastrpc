// Markup.cpp: implementation of the XMarkup class.
//
// Markup Release 10.0
// Copyright (C) 2008 First Objective Software, Inc. All rights reserved
// Go to www.firstobject.com for the latest XMarkup and EDOM documentation
// Use in commercial applications requires written permission
// This software is provided "as is", with no warranty.
//
#include <stdio.h>
#include "xcore_markup.h"
...
void XMarkup::operator=( const XMarkup& markup )
{
...
bool XMarkup::SetDoc( MCD_PCSZ pDoc )
{
	// Set document text
	if ( pDoc )
		m_strDoc = pDoc;
...
bool XMarkup::SetDoc( const MCD_STR& strDoc )
{
	m_strDoc = strDoc;
	MCD_STRCLEAR(m_strError);
	return x_ParseDoc();
...
bool XMarkup::Load( MCD_CSTR szFileName )
{
...
bool XMarkup::ReadTextFile( MCD_CSTR szFileName, MCD_STR& strDoc, MCD_STR* pstrError, int* pnDocFlags )
{
...
bool XMarkup::Save( MCD_CSTR szFileName )
{
	return WriteTextFile( szFileName, m_strDoc, &m_strError, &m_nDocFlags );
}

bool XMarkup::WriteTextFile( MCD_CSTR szFileName, MCD_STR& strDoc, MCD_STR* pstrError, int* pnDocFlags )
{
...
bool XMarkup::FindElem( MCD_CSTR szName )
{
...
bool XMarkup::FindChildElem( MCD_CSTR szName )
{
...
MCD_STR XMarkup::EscapeText( MCD_CSTR szText, int nFlags )
{
...
MCD_STR XMarkup::UnescapeText( MCD_CSTR szText, int nTextLength /*=-1*/ )
{
...
int XMarkup::x_WCToMB( int nCP, char* pszMB, int nMBCount, const wchar_t* pwszWide, int nWideLen, int* pnFailed/*=NULL*/ )
{
...
int XMarkup::UTF16To8( char* pszUTF8, const wchar_t* pwszUTF16, int nUTF8Count )
{
...
int XMarkup::DecodeCharUTF8( const char*& pszUTF8 )
{
...
void XMarkup::EncodeCharUTF16( int nUChar, wchar_t* pwszUTF16, int& nWideLen )
{
...
int XMarkup::x_MBToWC( int nCP, wchar_t* pwszWide, int nWideCount, const char* pszMB, int nMBLen )
{
#if defined(MARKUP_STDCONV)
...
int XMarkup::UTF8To16( wchar_t* pwszUTF16, const char* pszUTF8, int nUTF8Count )
{
...
int XMarkup::DecodeCharUTF16( const wchar_t*& pwszUTF16 )
{
...
void XMarkup::EncodeCharUTF8( int nUChar, char* pszUTF8, int& nUTF8Len )
{
...
MCD_STR XMarkup::UTF8ToA( MCD_CSTR pszUTF8, int* pnFailed/*=NULL*/ )
{
...
MCD_STR XMarkup::AToUTF8( MCD_CSTR pszANSI )
{
...
MCD_STR XMarkup::GetDeclaredEncoding( MCD_CSTR szDoc )
{
...
int XMarkup::FindNode( int nType )
{
...
MCD_STR XMarkup::GetTagName() const
{
...
MCD_STR XMarkup::GetAttribName( int n ) const
{
...
bool XMarkup::SavePos( MCD_CSTR szPosName /*=""*/, int nMap /*=0*/ )
{
...
bool XMarkup::RestorePos( MCD_CSTR szPosName /*=""*/, int nMap /*=0*/ )
{
...
bool XMarkup::SetMapSize( int nSize, int nMap /*=0*/ )
{
...
MCD_STR XMarkup::x_GetLastError()
{
...
bool XMarkup::x_AllocPosArray( int nNewSize /*=0*/ )
{
...
int XMarkup::x_ParseElem( int iPosParent, TokenPos& token )
{
...
bool XMarkup::x_FindAny( MCD_PCSZ pDoc, int& nChar )
{
...
bool XMarkup::x_FindName( XMarkup::TokenPos& token )
{
...
MCD_STR XMarkup::x_GetToken( const XMarkup::TokenPos& token )
{
...
int XMarkup::x_FindElem( int iPosParent, int iPos, MCD_PCSZ pPath ) const
{
...
int XMarkup::x_ParseNode( XMarkup::TokenPos& token, XMarkup::NodePos& node )
{
...
MCD_STR XMarkup::x_GetPath( int iPos ) const
{
	MCD_STR strPath;
	while ( iPos )
	{
...
MCD_STR XMarkup::x_GetTagName( int iPos ) const
{
...
bool XMarkup::x_FindAttrib( XMarkup::TokenPos& token, MCD_PCSZ pAttrib, int n/*=0*/ )
{
...
MCD_STR XMarkup::x_GetAttrib( int iPos, MCD_PCSZ pAttrib ) const
{
...
bool XMarkup::x_SetAttrib( int iPos, MCD_PCSZ pAttrib, int nValue, int nFlags /*=0*/ )
{
...
bool XMarkup::x_SetAttrib( int iPos, MCD_PCSZ pAttrib, MCD_PCSZ pValue, int nFlags /*=0*/ )
{
...
bool XMarkup::x_CreateNode( MCD_STR& strNode, int nNodeType, MCD_PCSZ pText )
{
...
MCD_STR XMarkup::x_EncodeCDATASection( MCD_PCSZ szData )
{
...
bool XMarkup::x_SetData( int iPos, int nValue )
{
	// Convert integer to string
	MCD_CHAR szVal[25];
	MCD_SPRINTF( MCD_SSZ(szVal), MCD_T("%d"), nValue );
...
bool XMarkup::x_SetData( int iPos, MCD_PCSZ szData, int nFlags )
{
...
MCD_STR XMarkup::x_GetData( int iPos ) const
{
	if ( iPos == m_iPos && m_nNodeLength )
	{
...
MCD_STR XMarkup::x_GetElemContent( int iPos ) const
{
...
bool XMarkup::x_SetElemContent( MCD_PCSZ szContent )
{
...
void XMarkup::x_DocChange( int nLeft, int nReplace, const MCD_STR& strInsert )
{
...
void XMarkup::x_Adjust( int iPos, int nShift, bool bAfterPos /*=false*/ )
{
...
int XMarkup::x_InsertNew( int iPosParent, int& iPosRel, XMarkup::NodePos& node )
{
...
bool XMarkup::x_AddElem( MCD_PCSZ pName, int nValue, int nFlags )
{
	// Convert integer to string
	MCD_CHAR szVal[25];
	MCD_SPRINTF( MCD_SSZ(szVal), MCD_T("%d"), nValue );
...
bool XMarkup::x_AddElem( MCD_PCSZ pName, MCD_PCSZ pValue, int nFlags )
{
	if ( nFlags & MNF_CHILD )
	{
...
MCD_STR XMarkup::x_GetSubDoc( int iPos ) const
{
	if ( iPos )
	{
...
bool XMarkup::x_AddSubDoc( MCD_PCSZ pSubDoc, int nFlags )
{
...
int XMarkup::x_RemoveElem( int iPos )
{
...
void XMarkup::x_LinkElem( int iPosParent, int iPosBefore, int iPos )
{
...
int XMarkup::x_UnlinkElem( int iPos )
{
...
int XMarkup::x_ReleasePos( int iPos )
{
...
int XMarkup::x_ReleaseSubDoc( int iPos )
{
...
bool XMarkup::x_GetMap( SavedPosMap*& pMap, int nMap, int nMapSize /*=7*/ )
{
...
void XMarkup::x_AdjustForNode( int iPosParent, int iPos, int nShift )
{
...
bool XMarkup::x_AddNode( int nNodeType, MCD_PCSZ pText, int nNodeFlags )
{
...
void XMarkup::x_RemoveNode( int iPosParent, int& iPos, int& nNodeType, int& nNodeOffset, int& nNodeLength )
{
...
// xml helper
///////////////////////////////////////////////////////////////////////////////
uint32 xml_count(XMarkup& xml, const string& path)
{
	string path_ = path;
	XStrUtil::chop(path_);
	vector<string> vItems;
	if (XStrUtil::split(path_, vItems, "|") <= 0) return 0;
	string last = vItems.back();
	vItems.pop_back();
	XStrUtil::chop(last, "\t\r\n ");
	if (last.empty()) return 0;

	xml.ResetPos();
	for (uint32 i = 0; i < (uint32)vItems.size(); i++)
	{
		if (i != 0 && !xml.IntoElem()) return 0;
		XStrUtil::chop(vItems[i]);
		if (vItems[i].empty() || vItems[i].at(0) == '@') return 0;
		XStrParser parser(vItems[i].c_str(), vItems[i].size());
		string token = parser.getstr_without("[");
		if (parser.eof())
		{
			if (!xml.FindElem(token)) return 0;
		}
		else
		{
			uint32 index = 0;
			string strNumber;
			if (!parser.getstr_by_sign(strNumber, '[', ']', false)) return 0;
			if (!parser.eof()) return 0;
			if (!XStrUtil::to_uint(strNumber, index)) return 0;
	...
	if (last.at(0) == '@')
	{
		...
		for (int i = 0; ; i++)
		{
			string name = xml.GetAttribName(i);
			if (name.empty()) break;
			if (name == last) count++;
		}
		return count;
	...
	{
		XStrParser parser(last.c_str(), last.size());
		string token = parser.getstr_without("[");
		if (parser.eof())
		{
			uint32 count = 0;
			if (!vItems.empty() && !xml.IntoElem()) return 0;
			while (xml.FindElem(token)) count++;
			return count;
		}
		else
		{
...
bool xml_get(XMarkup& xml, const string& path, string& val)
{
	val.clear();
	string path_ = path;
	XStrUtil::chop(path_);
	vector<string> vItems;
	if (XStrUtil::split(path_, vItems, "|") <= 0) return false;
	string last = vItems.back();
	vItems.pop_back();
	XStrUtil::chop(last, "\t\r\n ");
	...
	for (uint32 i = 0; i < (uint32)vItems.size(); i++)
	{
		if (i != 0 && !xml.IntoElem()) return false;
		XStrUtil::chop(vItems[i]);
		if (vItems[i].empty() || vItems[i].at(0) == '@') return false;
		XStrParser parser(vItems[i].c_str(), vItems[i].size());
		string token = parser.getstr_without("[");
		if (parser.eof())
		{
			if (!xml.FindElem(token)) return false;
		}
		else
		{
			uint32 index = 0;
			...
			if (!XStrUtil::to_uint(strNumber, index)) return false;
			for (uint32 i = 0; i <= index; i++)
			{
				if (!xml.FindElem(token)) return false;
			}
	...
	else
	{
		...
		if (parser.eof())
		{
			if (!vItems.empty() && !xml.IntoElem()) return false;
			if (!xml.FindElem(token)) return false;
			val = xml.GetData();
			return true;
		}
...
bool xml_set(XMarkup& xml, const string& path, const string& val)
{
	string path_ = path;
	XStrUtil::chop(path_);
	vector<string> vItems;
	...
	for (uint32 i = 0; i < (uint32)vItems.size(); i++)
	{
		...
		if (parser.eof())
		{
			...
			{
				if (!xml.AddElem(token)) return false;
			}
		}
		else
		{
...
bool xcore_test_markup()
{
	...
	xml.AddElem("Message");
	xml.IntoElem();
	xml.AddElem("SubMessage");
	xml.AddAttrib("Name", str);
	printf("doc:\n%s", xml.GetDoc().c_str());
	printf("Name:%s\n", xml.GetAttrib("Name").c_str());
	return true;
}
