...
#define MARKUP_STL	// use STL string
#undef _MBCS
...
public:
	...
	XMarkup() { SetDoc( NULL ); InitDocFlags(); };
	XMarkup( MCD_CSTR szDoc ) { SetDoc( szDoc ); InitDocFlags(); };
	XMarkup( int nFlags ) { SetDoc( NULL ); m_nDocFlags = nFlags; };
	XMarkup( const XMarkup& markup ) { *this = markup; };
	void operator=( const XMarkup& markup );
	~XMarkup() {};
...
// xml helper
///////////////////////////////////////////////////////////////////////////////
// path:  message|sub1|sub2[N]|sub3
// path:  message|sub1|sub2[N]|sub3|@attr
// path:  message|sub1|sub2[N]|sub3[N]
