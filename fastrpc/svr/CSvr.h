...
typedef int StartFunction(int listen_fd);

extern int StartWithMultProc(std::string ip, unsigned short port, StartFunction* fun, int proc_num = -1);

extern int StartOneProc(int listen_sd, int sock_num,
                    int io_thread_num, int worker_num, void* rpc_param=NULL);

extern int StartMulThread(std::string& ip, unsigned short port, int sock_num,
                    int io_thread_num, int worker_num, void* rpc_param=NULL);
