...
{
    HttpResponse req1;
    req1.set_http_version(HttpMessage::VERSION_1_0);
    req1.SetHeader("A", "1");
    req1.set_body("hello");

    HttpResponse req2;
    req2.set_http_version(HttpMessage::VERSION_1_1);
    req2.SetHeader("B", "1");
    req2.set_body("world");

    std::swap(req1, req2);

    EXPECT_EQ(HttpMessage::VERSION_1_1, req1.http_version());
    EXPECT_FALSE(req1.HasHeader("A"));
    EXPECT_TRUE(req1.HasHeader("B"));
    EXPECT_EQ("world", req1.http_body());

    EXPECT_EQ(HttpMessage::VERSION_1_0, req2.http_version());
...
{
    HttpResponse response;
    response.set_http_version(HttpMessage::VERSION_1_0);
    EXPECT_FALSE(response.IsKeepAlive());
    response.set_http_version(HttpMessage::VERSION_0_9);
    EXPECT_FALSE(response.IsKeepAlive());
    response.set_http_version(HttpMessage::VERSION_UNKNOWN);
    EXPECT_FALSE(response.IsKeepAlive());

    response.set_http_version(HttpMessage::VERSION_1_1);
