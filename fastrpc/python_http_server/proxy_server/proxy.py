...
def clearfdpro(epoll_fd, params, fd):
    ...
    try:
        param = params[fd]
        if "next" in param:
            print "this is next client"
        else:
            print "close master client"
                        ...
                        if msg.errno == errno.EAGAIN:
                            if not "nextfd" in param:
                                ...
                                if host_s > 0 and host_e > 0:
                                    ...
                                    try:
                                        print "cont: %s" % datas
                                        if datas.startswith("GET") or \
                                                datas.startswith("POST") or \
                                                datas.startswith("get") or \
                                                datas.startswith("post"):
                                            ...
                                            break
                                        #elif datas.startswith("CONNECT"):
                                        #    nextfd = ssl.wrap_socket(socket.socket())
                                        #    nextfd.connect((host, 443))
                                        #    nextfd.setblocking(0)
                                        #    next_fileno = nextfd.fileno()
                                        #    print "pid:%s, connect %s:%s fd:%s" % (os.getpid(), host, port, next_fileno)
                                        #    if next_fileno in params:
